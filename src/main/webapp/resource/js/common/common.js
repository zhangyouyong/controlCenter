(function($) {
	...
	$.fn.selectInit = function(options) {
		...
		if (options.defaults) {
			$(this).append(
					"<option value=" + options.defaults.key + ">"
							+ options.defaults.value + "</option>");
		}
	...
	$.fn.addressCommon = function(options) {
		...
		// var
		// checkData=$.getAjaxHtmlData("addressGanged/datailAddress.htm?addressId="+$(obj).val());
		$(this).prepend("<span tag=''>" + title + ":</span>");
		$(this).append(
				"<select tag='province' onchange='checkAddress(this)' name='"
						+ isNotEmpty(options.nodeName.provinceName)
						+ "'><option value=''>--请选择--</option></select>省 ");
		$(this).append(
				"<select tag='city' onchange='checkAddress(this)' name='"
						+ isNotEmpty(options.nodeName.cityName)
						+ "'><option value=''>--请选择--</option></select>市 ");
		$(this).append(
				"<select tag='district' name='"
						+ isNotEmpty(options.nodeName.districtName)
						+ "'><option value=''>--请选择--</option></select>区 ");
		if (type = "addressAndShop") {
			$(this).append(
					"<select tag='shop' name='"
							+ isNotEmpty(options.nodeName.shopName)
							+ "'><option>--请选择--</option></select>");
			$(this).next().prepend("<option value=''></option>");
			$(this)
					.find("select[tag='district']")
					.change(
							function() {
								$(this).next().empty().prepend(
										"<option value=''>--请选择--</option>");
								var dataMarket = $
										.getAjaxHtmlData("rafflesShop/getMarketData.htm?id="
												+ $(this).val());
								for (var i = 0; i < dataMarket.rows.length; i++) {
									$(this).next().append(
											"<option value="
													+ dataMarket.rows[i].id
													+ ">"
													+ dataMarket.rows[i].name
													+ "</option>");
								}
							});
		}
		for (var i = 0; i < initData.rows.length; i++) {
			$(this).find("select[tag='province']").append(
					"<option value='" + initData.rows[i].id + "'>"
							+ initData.rows[i].name + "</option>");
		}
	...
	$.fn.showbox = function(options) {
		...
		var $htmls = "<div class='showbox' style='position:fixed;top:0;left:0;background:rgba(0,0,0,0.1)!important;z-index:"
				+ data.zindex
	...
	$.newTabWindow = function(url) {
		var linking = $("<a href='" + url + "' target='_blank'>Hiaas</a>").get(
				0);
	...
	$.fn.Ynotify = function messagebox(message, type, delay) {
		...
		var Box = {
			...
			notify : function() {
				...
				var str = "<div id=\"y_notify\" class=\"" + box_class
						+ "\" style=\"" + box_style + "\">" + message
						+ "</div>";
				$("body").append(str);
				var ext_width = $("#y_notify").width();

				$("#y_notify").stop().animate({
					top : 2
				});
...
function checkAddress(obj) {
	var data = $.getAjaxHtmlData("addressGanged/datailAddress.htm?addressId="
			+ $(obj).val());
	var nextNode = $(obj).nextAll().empty().prepend(
			"<option value=''>--请选择--</optio>");
